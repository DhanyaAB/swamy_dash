...
import { buildtwoservice } from './private.service';
...
export class PrivateComponent implements OnInit, OnDestroy {
  ...
  constructor(private activatedRoute: ActivatedRoute,
    ...
    private _buidtwoService: buildtwoservice,
  ...
  getBuildNo() {
    var mainData;
    this._buidtwoService.getBuildNo().subscribe(
      data => {
        mainData = data;
  ...
  getProgressBarData() {
    ...
    this.subscription = this._buidtwoService.getProgressBarData(buildname, this.userId, this.testcase).subscribe(
      response => {
        ...
        for (let _progressData of this.progressData) {
          ...
          //if (_progressData.name === "Setup Jenkis Env" || _progressData.name === "Generating Build") {
  ...
  getBuildResponse() {
    var tempData;
    var buildname = "Build_Generator_ssa";
    this._buidtwoService.getBuildResponse(this.userId).subscribe(
      response => {
        tempData = response;
        ...
        //  alert("SUCCESS");
          ...
          this.toggleView = true;
          this.getValuefromImageId(this.userId)
          // this._buidtwoService.getBuildStatus(buildname, this.userId).subscribe(
          //   response => {
  ...
  getConsoleData() {
    ...
    this._buidtwoService.getBuildNo().subscribe(
      data => {
        ...
        this._buidtwoService.getConsoleData(buildname, this.userId).subscribe(
          data => {
            tempdata = data;
